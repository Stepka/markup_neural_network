...
def get_onehot(paragraph):
    ...
    paragraph_onehot = [np.zeros(config['clustering']['one_hot_size'], dtype=np.int32)]

    for i in range(len(paragraph)):
        word = paragraph[i]
        # есть ли слово в модели? Может быть, и нет
        onehot = np.zeros(config['clustering']['one_hot_size'], dtype=np.int32)
...
def clusterize_vocab():
    ...
    if START_STAGE >= 2:
        ...
        if not os.path.exists(config['clustering']['saved_model'] + '/kmeans.joblib'):
            raise Exception('Wrong start stage', 'KMeans model with clustered embeddings does not exist by path ' +
                            config['clustering']['saved_model'] + '/kmeans.joblib' +
                            ', please, add file or decrease start stage to "clusterize_vocab" or previous.')

        model = joblib.load(config['clustering']['saved_model'] + '/kmeans.joblib')
        ...
        model = KMeans(n_clusters=config['clustering']['one_hot_size'], verbose=1)
        model.fit(vocab_embeddings)

        joblib.dump(model, config['clustering']['saved_model'] + '/kmeans.joblib')
...
def train():
    ...
    # Specify feature
    feature_columns = [tf.feature_column.numeric_column("x", shape=[config['clustering']['one_hot_size']], dtype=tf.dtypes.int32)]

    classifier_model = tf.estimator.DNNClassifier(
        hidden_units=[512, 256, 128],
        feature_columns=feature_columns,
        n_classes=num_classes,
        model_dir=config['classifier']['saved_model']
    ...
    if START_STAGE >= 3:
        # Load model
        if len(os.listdir(config['classifier']['saved_model'])) == 0:
            raise Exception('Wrong start stage', 'Classifier model does not exist by path ' +
                            config['classifier']['saved_model'] +
        ...
        for the_file in os.listdir(config['classifier']['saved_model']):
            file_path = os.path.join(config['classifier']['saved_model'], the_file)
            try:
                if os.path.isfile(file_path):
                    os.unlink(file_path)
            except Exception as e:
